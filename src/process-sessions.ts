...
import type { VerificationType } from "./operations/operation-contracts.js";
...
export interface ProcessVerificationTarget {
  runId: string;
  workspaceId: string;
...
export interface ProcessSessionProjection {
  started(input: {
    sessionId: number;
    workspaceId: string;
    tty: boolean;
...
export class ProcessSessionManager {
  ...
  terminate(workspaceId: string, sessionId: number): boolean {
    ...
    if (session.running) {
      ...
      if (!projected) {
        throw new Error("Process stop could not be projected.");
      }
      try {
        session.process?.kill("SIGTERM");
      } catch (error) {
        this.project(() => this.projection?.stopFailed?.({
          sessionId: session.id,
          workspaceId: session.workspaceId,
          reason: "terminate",
        }));
  ...
  getSessionStatus(
    ...
    if (!session || session.workspaceId !== workspaceId) return "missing";
    return session.running ? "running" : "completed";
  }
  ...
  private startPipe(session: ProcessSession, input: StartCommandInput): void {
    ...
    this.projectStarted(session, Boolean(input.tty));
    child.stdout.on("data", (data: Buffer) =>
      this.append(session, data.toString("utf8"), "stdout"));
    child.stderr.on("data", (data: Buffer) =>
      this.append(session, data.toString("utf8"), "stderr"));
  ...
  private async startPty(session: ProcessSession, input: StartCommandInput): Promise<void> {
    ...
    this.projectStarted(session, true);
    pty.onData((data) => this.append(session, data, "combined"));
  ...
  private append(
    ...
    this.project(() => this.projection?.output({
      sessionId: session.id,
      workspaceId: session.workspaceId,
      stream,
      text: output,
  ...
  private project(project: () => void): void {
    try {
      project();
    } catch {
  ...
  private projectStarted(session: ProcessSession, tty: boolean): void {
    this.project(() => this.projection?.started({
      sessionId: session.id,
      workspaceId: session.workspaceId,
      tty,
      verification: session.verification,
