...
function parseRequiredSecret(value: string | undefined, name: string): string {
  ...
  if (!secret) {
    throw new Error(`${name} is required for dpkr helix OAuth. Run: devspace init`);
  }
  if (secret.length < 16) {
  ...
  return {
    ...
    dashboard: {
      enabled: parseOptionalBoolean(
        env.DEVSPACE_DASHBOARD,
        files.config.dashboardEnabled ?? true,
      ),
      host: "127.0.0.1",
      port: parsePort(env.DEVSPACE_DASHBOARD_PORT ?? files.config.dashboardPort ?? port + 1),
      token: parseOptionalSecret(env.DEVSPACE_DASHBOARD_TOKEN ?? files.auth.dashboardToken, "DEVSPACE_DASHBOARD_TOKEN"),
    },
