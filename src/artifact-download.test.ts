...
import { workspaceFileMutationPathDeniedError } from "./projects/project-policy.js";
import {
...
function testLogRedaction(): void {
  ...
  const deniedFields = artifactToolFailureLogFields(
    {
      file: {
        download_url: "https://tenant-secret.example/file?sig=denied-secret",
        file_id: "file_denied_secret",
        authorization: "Bearer denied-log-secret",
      },
  ...
  assert.deepEqual(deniedFields, { workspaceId: "ws_denied" });
  const deniedSerialized = JSON.stringify(deniedFields);
  assert.equal(deniedSerialized.includes("tenant-secret"), false);
  assert.equal(deniedSerialized.includes("file_denied_secret"), false);
  assert.equal(deniedSerialized.includes("client-secret"), false);
  assert.equal(deniedSerialized.includes("Bearer"), false);
}
