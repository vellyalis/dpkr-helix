...
import { attachMcpToolResultProjection } from "./operations/mcp-tool-operation-projector.js";
...
export function registerArtifactTools(
  ...
  registerAppTool(
    ...
    async (input) => {
      const response = await executeArtifactTool(config, input, async () => {
      ...
      return attachMcpToolResultProjection(response, {
        fileChanges: [{
          relativePath: structured.path,
          operation: "create",
        }],
      });
    },
...
export function artifactToolFailureLogFields(
  input: Record<string, unknown>,
  error: unknown,
): Record<string, unknown> {
  ...
  return artifactToolLogFields(input);
}
  ...
  try {
    ...
    if (config.logging.toolCalls) {
      logEvent(config.logging, "warn", "artifact_tool_call", {
        tool: "download_artifact",
        ...artifactToolFailureLogFields(input, error),
        success: false,
        errorCode:
