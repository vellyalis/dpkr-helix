...
## Registered Project Permission Presets

The local dashboard owns permission changes for registered projects. Remote MCP
tools cannot change a preset.

| Preset | Intended use | Allowed side effects |
| --- | --- | --- |
| `inspect` | Read-only investigation | Read, search, and list only |
| `design` | Documentation and planning | Writes only to approved documentation paths |
| `develop` | Implementation | Workspace changes, shell, artifacts, and workspace-write delegation |

Unregistered legacy workspaces retain their existing behavior. A preset limits
DevSpace-owned operations; it is not a sandbox for arbitrary shell commands.
