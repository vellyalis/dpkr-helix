...
## Live Operations And Verification

The local dashboard projects activity from the existing MCP, process, agent,
Git/change-review, and verification owners. It observes work; it does not start
a duplicate execution.

`Result available — verification pending` means output exists but has not passed
the required checks. `Verifying` means those checks are running. Only evidence
from the actual check can move the run to `Verified`. `Stopped` means execution
was asked to stop; it never means file changes were rolled back.
