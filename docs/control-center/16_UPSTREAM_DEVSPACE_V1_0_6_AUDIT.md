...
| Waiting MCP sessions dominate `NOW` | A live state inspection found 63 of 64 retained running MCP session operations in phase `waiting`, not performing a tool action. The current queue derives every running state as `NOW`. | **Next Operations/UI candidate after live deployment.** Do not mislabel waiting sessions as archived. Compare bounded idle completion against an explicit standby presentation using a real dashboard trace. |
| Workspace-session retention | The installed database contained 1,033 workspace-session rows, all marked active, while the entire state database remained about 6.7 MiB. Conversation reuse has not yet been deployed, so its effect on the creation rate is unknown. | **Measure before changing.** Deploy reuse, report counts and growth in System diagnostics, then add bounded retention only if growth remains material. Do not delete old continuity state merely to reduce a currently small database. |
