...
| Provider quota visibility | A live worker reached Codex but failed only after launch because the provider usage allowance was exhausted; package/executable availability still reported Codex usable. | **Adopted as bounded cooldown.** Derive reset time and structured failure from the existing agent row, block duplicate launches before side effects, and never switch provider automatically. |
| Workspace-session retention | A later live read found 1,072 sessions across 242 roots, with 149 created in 24 hours, 944 in seven days, and only 19 conversation bindings. All rows still reported active. | **Adopted as diagnostics plus reversible local archive.** Show growth first; archive only old unbound checkout index rows after transactional active-work rechecks. Never delete files or worktrees, and reactivate on reuse. |
| New-chat resume and failure diagnosis | Project, Handoff, Git, workspace, Operations, agent and verification facts existed but required multiple calls and manual correlation after a chat switch or failure. | **Adopted as one read-only projection.** Add `get_project_resume` and the Project inspector Current/Resume section over existing owners; redact latest failures and return one concrete next action. |
